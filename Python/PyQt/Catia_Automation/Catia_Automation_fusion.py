...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        #전체 Update 버튼
        btn_Update_all = QPushButton('Refresh')
        ...
        grid.addWidget(self.PLM_Search_Query_Group(),4,0)
        grid.addLayout(h_box_bottom,8,0)

        self.setLayout(grid)

        self.setWindowTitle('Catia Automation Tools (Ver 0.27)')
    ...
    def Search_and_Select(self, Search_Text):
        ...
        Active_Document_Selection.Search(Search_Query)
        count = Active_Document_Selection.count
    ...
    def Get_Origin(self) :
        view_point_3D = pycatia.active_window.active_viewer.create_viewer_3d().viewpoint_3d
    ...
    def PLM_Search_Query_Group(self) :
        groupbox = QGroupBox('PLM Search Query')

        V_Box_PLM_Part_No = QVBoxLayout()

        Label_Part_No = QLabel("Part Number")

        self.LE_Part_No = QLineEdit()
        self.LE_Part_No.returnPressed.connect(self.append_text_Part_No)

        self.TB_Part_No = QTextBrowser()
        self.TB_Part_No.setAcceptRichText(True)
        self.TB_Part_No.setOpenExternalLinks(True)

        self.clear_btn_Part_No = QPushButton('Clear')
        self.clear_btn_Part_No.pressed.connect(self.clear_text_Part_No)

        V_Box_PLM_Part_No.addWidget(Label_Part_No, 0)
        V_Box_PLM_Part_No.addWidget(self.LE_Part_No, 1)
        V_Box_PLM_Part_No.addWidget(self.TB_Part_No, 2)
        V_Box_PLM_Part_No.addWidget(self.clear_btn_Part_No, 3)

        V_Box_PLM_VCode = QVBoxLayout()

        Label_VCode = QLabel("Vehicle Code")

        self.LE_VCode = QLineEdit()
        self.LE_VCode.returnPressed.connect(self.append_text_VCode)

        self.TB_VCode = QTextBrowser()
        self.TB_VCode.setAcceptRichText(True)
        self.TB_VCode.setOpenExternalLinks(True)

        self.clear_btn_VCode = QPushButton('Clear')
        self.clear_btn_VCode.pressed.connect(self.clear_text_VCode)

        V_Box_PLM_VCode.addWidget(Label_VCode, 0)
        V_Box_PLM_VCode.addWidget(self.LE_VCode, 1)
        V_Box_PLM_VCode.addWidget(self.TB_VCode, 2)
        V_Box_PLM_VCode.addWidget(self.clear_btn_VCode, 3)

        H_Box_PLM_LE_TB = QHBoxLayout()

        H_Box_PLM_LE_TB.addLayout(V_Box_PLM_Part_No)
        H_Box_PLM_LE_TB.addLayout(V_Box_PLM_VCode)

        V_Box_PLM_Lv2 = QVBoxLayout()

        self.LE_PLM_Query = QLineEdit()
        Button_Query_Gen = QPushButton("Generate PLM Search query")
        Button_Query_Gen.clicked.connect(self.Gen_Search_Query)

        V_Box_PLM_Lv2.addLayout(H_Box_PLM_LE_TB)
        V_Box_PLM_Lv2.addWidget(Button_Query_Gen)
        V_Box_PLM_Lv2.addWidget(self.LE_PLM_Query)

        H_Box_PLM_Main = QHBoxLayout()

        str_How_to = "◎ Part number Example :"
        str_How_to += "\n    - 663*"
        str_How_to += "\n    - 663??"
        str_How_to += "\n    - 64??0"
        str_How_to += "\n\n◎ Vehicle Code Example :"
        str_How_to += "\n    - P1"
        str_How_to += "\n    - P19"
        str_How_to += "\n    - P1000"
        str_How_to += "\n    - P1?3"
        ...
        PLM_Query_How_to = QLabel(str_How_to)
        H_Box_PLM_Main.addLayout(V_Box_PLM_Lv2)
        H_Box_PLM_Main.addWidget(PLM_Query_How_to)

        groupbox.setLayout(H_Box_PLM_Main)
        return groupbox

    def append_text_Part_No(self):
        text = self.LE_Part_No.text()
        if (len(text) == 5) or (len(text)<=5 and text[-1:]=="*") :
            self.TB_Part_No.append(text)
            self.LE_Part_No.clear()

    def clear_text_Part_No(self):
        self.TB_Part_No.clear()

    def append_text_VCode(self):
        text = self.LE_VCode.text()
        if len(text)>=1 and len(text)<=5 :
            self.TB_VCode.append(text)
            self.LE_VCode.clear()

    def clear_text_VCode(self):
        self.TB_VCode.clear()

    def Gen_Search_Query(self) :
        str_PLM_Search_Query = ""
        VCode_Arr=[]
        Part_No_Arr=[]
        VCode_Arr = self.TB_VCode.toPlainText().split("\n")
        Part_No_Arr = self.TB_Part_No.toPlainText().split("\n")
        if len(VCode_Arr[0])*len(Part_No_Arr[0]) >=1 :
            for i in VCode_Arr :
                for j in Part_No_Arr :
                    str_PLM_Search_Query = str_PLM_Search_Query + j + i +"*.CATPart; "
            self.LE_PLM_Query.setText(str_PLM_Search_Query)
        else :
            self.LE_PLM_Query.setText("No Inputs")
...
if __name__ == '__main__':
    ...
    # palette.setColor(QPalette.Window, QColor(53,51,102))
